...
function textImage(text,noClean){
	...
	var s=new Sanitize(text);
		text=s.removeWhiteSpace()
			...
			.getString();
		if(heading){
			s=new Sanitize(heading);
			heading=s.toSentenceCase().getString();
		}
	}
	...
	ctx.stroke();
	var imgURL=Math.round(Math.random()*3)+1;
	console.log(imgURL);
	drawImage('images/'+imgURL+'.jpg',ctx)
			.then(function(resp)
			{
				defer.resolve(resp)
			}).catch(function(err)
...
function drawImage(imgURL,ctx){
	var defer=Q.defer();
	fs.readFile(__dirname+'/../'+imgURL,function(err,i)
		{
			if(err){
